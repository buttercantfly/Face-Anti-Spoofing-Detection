...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        return {'image_x': torch.from_numpy(image_x.astype(np.float64)).float(), 'map_x': torch.from_numpy(map_x.astype(np.float64)).float(), 'spoofing_label': torch.from_numpy(spoofing_label_np.astype(np.long)).long()}
...
class Spoofing_train(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if spoofing_label == 1:
            ...
            spoofing_label = 0
            # print('frame total = ', frames_total,'\n')
            ...
            for a in range(frames_total):
                ...
                if type(image_id) is int:
                    ...
                    # s = "_%03d_scene" % 1
                    image_name1 = videoname + s + '.jpg'
                    image_path1 = os.path.join(image_path, image_name1)
            ...
            image_x = seq.augment_image(image_x_temp)
            map_x = np.zeros((32, 32))    # fake
        # print(videoname, ' is ok\n')
    ...
    def get_single_image_x(self, image_path, map_path, videoname):
        ...
        # 這個 for 迴圈拿來定義名字跟檢查資料缺失
        for temp in range(60):
            # generate number between 1 ~ frame_total-1
            image_id = np.random.randint(1, 60)
            ...
            if type(image_id) is int:
                ...
                if os.path.exists(image_path2) and os.path.exists(map_path2):
                    # using 0 to get gray scale mode
                    map_x_temp2 = cv2.imread(map_path2, 0)
                    image_x_temp2 = cv2.imread(image_path2)
                    if image_x_temp2 is not None and map_x_temp2 is not None:
                        # print('gonna break ////////////////////////////')
                        break
        ...
        #     print('fuck you ezikkile\n')
        # else:
        #     print('image x temp is ok\n')

        # gray-map
        map_x_temp = cv2.imread(map_path2, 0)
        ...
        if image_x_temp.shape != (256, 256, 3):
            ...
            print('error!!!!!!!!!!!!!!!!!!!!!!!!!!!')
            image_x_temp = cv2.resize(image_x_temp, (256, 256))

        # data augment from 'imgaug' --> Add (value=(-40,40), per_channel=True), GammaContrast (gamma=(0.5,1.5))
        # 讓image有一點顏色變化
        image_x_aug = seq.augment_image(image_x_temp)

        if map_x_temp.shape == (256, 256):
            map_x_temp = cv2.resize(map_x_temp, (32, 32))
        else:
            print(map_x_temp.shape)
            print('map size is wrong ')
            print('error!!!!!!!!!!!!!!!!!!!!!!!!!!!')


        return image_x_aug, map_x_temp
