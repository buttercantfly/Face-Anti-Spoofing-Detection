...
# face_scale有點像是放大input的區域(從基礎給的bounding box上)
# 應該也算是augmentation的一部份(讓model對於不同大小的臉都可以去辨識出來)
...
class Spoofing_valtest(Dataset):
    ...
    def get_single_image_x(self, image_path, val_map_path, videoname):
        ...
        # frames_total = 8 (每個影片都抓八個出來)
        image_x = np.zeros((frames_total, 256, 256, 3))
        val_map_x = np.ones((frames_total, 32, 32)) # vs mpa_x = np.zeros(32, 32)
        ...
        if os.path.exists(val_map_path):
            ...
            for ii in range(frames_total):
                ...
                for temp in range(50):
                    ...
                    if os.path.exists(val_map_path2)  :    # some scene.dat are missing
                ...
                # transform to binary mask --> threshold = 0 => 用來算分數的?

                temp = cv2.resize(val_map_x_temp, (32, 32))
                np.where(temp < 1, temp, 1) # 這句有點不懂 np.where(condition, x, y)? temp各點位置如果小於1是原值 其他則是1?
                val_map_x[ii,:,:] = temp
        else:
            for ii in range(frames_total):
                image_id = np.random.randint(1, frames_total-1)
                s = "-%04d.jpg" % image_id
                image_name = videoname + s
