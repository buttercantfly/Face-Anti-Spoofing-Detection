...
class Spoofing_train(Dataset):
    ...
    # in train_CDCN.py train_test called:
    # train_list, train_image_dir, map_dir
    # transforms.Compose([RandomErasing(), RandomHorizontalFlip(),  ToTensor(), Cutout(), Normaliztion()])
    ...
    # dataloader會用idx跑過list去讀資料
    def __getitem__(self, idx):
        #print(self.landmarks_frame.iloc[idx, 0])
        videoname = str(self.landmarks_frame.iloc[idx, 0]) # 原本1 (data_list.txt中是擺反ㄉ所以轉過來)
        image_path = os.path.join(self.root_dir, videoname)
        map_path = os.path.join(self.map_dir, videoname)

        # 用get_single_image_x抓圖(image and map)
        image_x, map_x = self.get_single_image_x(image_path, map_path, videoname)

		# landmarks_frame從list files裡面(protocol 裡面的.txt)抓spoofing_label
        spoofing_label = self.landmarks_frame.iloc[idx, 1] # 原本0
        if spoofing_label == 1:
    ...
    def get_single_image_x(self, image_path, map_path, videoname):
        ...
        print('')
        print("Video: ", videoname, " total frame is ", frames_total)
        print('')

        # random choose 1 frame and extract frame's map from map directory

        if videoname.split('-')[2] == '1':  # 如果是live的話才去抓檔案 否則用預設的32*32(0)作為map
            print('')
            print("Video is live")
            print('')
            for temp in range(500):
                image_id = np.random.randint(1, frames_total-1) # 隨機選擇一禎

                s = "-%04d" % image_id # "_%03d_scene" for OULU
                image_name = videoname + s + ".jpg" # image位置

                print('')
                print("Frame selected: ", image_name)
                print('')

                s = "-%04d_depth1D" % image_id
                map_name = videoname + s + ".jpg"
                map_path2 = os.path.join(map_path, map_name)  # map位置(用來確認map是否存在的)

                print('')
                print("Frame selected: ", map_name)
                print('')

                # some .dat & map files have been missing 如果有就break(找到) 沒有就繼續找
            ...
            map_x = cv2.resize(map_x_temp, (32, 32)) # 將map轉成32*32
        else:
            print('')
            print("Video is fake")
            print('')
            ...
            map_x = np.zeros((32, 32))  # map_x預設為0 (沒有深度)
        ...
        ***不懂這scale的意義*** (原本是用在哪?)
        ...
        # 這邊因為圖都已經擷取好了，所以不用crop_face_form_scene (所以也不用.dat檔案去框臉出來)
        image_x = cv2.resize(image_x_temp, (256, 256))
        # data augment
        # from 'imgaug' --> Add (value=(-40,40), per_channel=True), GammaContrast (gamma=(0.5,1.5))
        image_x_aug = seq.augment_image(image_x)

        # 回傳 image(256, 256, 3) 與 map (32, 32)
